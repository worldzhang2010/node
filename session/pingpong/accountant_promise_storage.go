...
func (aps *AccountantPromiseStorage) Store(id identity.Identity, accountantAddress common.Address, promise AccountantPromise) error {
	aps.lock.Lock()
	defer aps.lock.Unlock()

	channel, err := crypto.GenerateProviderChannelID(id.Address, accountantAddress.Hex())
	if err != nil {
...
func (aps *AccountantPromiseStorage) Get(id identity.Identity, accountantAddress common.Address) (AccountantPromise, error) {
	aps.lock.Lock()
	defer aps.lock.Unlock()
